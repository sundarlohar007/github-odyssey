...
import fs from 'fs';
export async function cardCommand(username, options) {
    ...
    try {
        ...
        const svg = generatePlayerCard(userData.user.login, userData.user.avatar_url, level, xp, nextLevelXp - currentLevelXp, userData.currentStreak, tier, earned.map(e => e.icon));
...
function generatePlayerCard(username, avatarUrl, level, xp, xpToNext, streak, tier, badges) {
